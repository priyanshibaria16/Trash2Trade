...
const normalizeCoords = (row) => {
  ...
  if (invalid) {
    return { ...row, latitude, longitude: null };
  }
 ...
 */
export const getPickupsByUserId = async (userId)=> {
  const query = `
    SELECT 
      p.*, 
      u.name as user_name,
 ...
 * @returns Array of pickups with user details
 */
export const getPickupsByCollectorId = async (collectorId)=> {
  const query = `
    SELECT 
      p.*, 
      u.name as user_name,
 ...
 */
export const getUserPickupStats = async (userId)=> {
  const query = `
    SELECT 
