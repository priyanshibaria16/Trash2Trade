  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Calculate stats
        const completedPickups = pickupsResponse.pickups?.filter((p) => p.status === 'completed') || [];
        const totalEarnings = earningsResponse.payments?.reduce((sum, earning) => 
          earning.status === 'completed' ? sum + earning.amount : sum, 0) || 0;
        const greenCoinsEarned = completedPickups.reduce((sum, pickup) => 
